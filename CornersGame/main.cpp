...
vector<Figure> InitializeFigures(const Texture &texture, const IntRect &rectangle, const Vector2i &initial_position,
	...
	for (int i = 0; i < kFigureRows; ++i) {
		for (int j = 0; j < kFigureColumns; ++j) {
			Figure f(texture, rectangle, Vector2f(initial_position.x + i * kCellSizeX, initial_position.y + j * kCellSizeY));
			figures.push_back(f);
			if (is_left_top) {
		...
		while (window.pollEvent(event))
		{
			...
			if (event.type == Event::MouseButtonPressed) {
				if (event.key.code == Mouse::Left) {
					...
					for (size_t i = 0; i < ai_figures.size(); ++i) {
						auto rect = ai_figures[i].GetSprite().getGlobalBounds();
						if (ai_figures[i].GetSprite().getGlobalBounds().contains(mouse_position.x, mouse_position.y)) {
							n = i;
							is_move = true;
							is_white = true;
							new_figure = person_figures[i];
			...
			if (event.type == Event::MouseButtonReleased) {
				if (event.key.code == Mouse::Left) {
					...
					dx = abs(Mouse::getPosition(window).x - old_position.x);
					dy = abs(Mouse::getPosition(window).y - old_position.y);
