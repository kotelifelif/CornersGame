...
AI::AI(const bool is_black)
    : is_black_(is_black), figure_number_(0), kMaxDepth(1) {}
...
int AI::FindBestMove(Board& board, Player& player, int depth) {
  ...
  for (size_t i = 0; i < figures.size(); ++i) {
    ...
    sf::Vector2i figure_cell_coordinates =
        board.GetCellCoordinates(figure_cell);

    if (figure_cell_coordinates.x - 1 >= 0) {
      sum_distance = CheckMove(board, player, figure_cell_coordinates,
                               sf::Vector2i(figure_cell_coordinates.x - 1,
    ...
    if (figure_cell_coordinates.y - 1 >= 0) {
      sum_distance = CheckMove(board, player, figure_cell_coordinates,
                               sf::Vector2i(figure_cell_coordinates.x,
                                            figure_cell_coordinates.y - 1),
                               depth, sum_distance, i, score);
    }
...
std::pair<sf::Vector2i, int> AI::GetMove() const {
...
int AI::CheckMove(Board& board, Player& player, sf::Vector2i& old_coordinates,
  ...
  if (!board.GetCell(new_coordinates.x, new_coordinates.y).is_busy) {
    ...
    player.SetFigurePosition(board, figure_number, new_coordinates.x,
                             new_coordinates.y);
    ...
    player.SetFigurePosition(board, figure_number, old_coordinates.x,
                             old_coordinates.y);
  }
  return sum_distance;
}
