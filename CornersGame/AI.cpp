...
std::pair<sf::Vector2i, int> AI::FindBestMove(Board& board, Player& player)
{
    ...
    int shortest_path_length = kRows * kColumns;
    int figure_number = -1;
    int evaluation = 0;
    Cell figure_cell;
...
std::vector<Cell> AI::GetShortestPath(Board& board, Cell source_cell, Cell destination_cell)
{
    ...
    int max = kRows * kColumns;
    std::vector<std::vector<int>> distances(adjacent_cells.size(), std::vector<int>(adjacent_cells.size(), max));
    std::vector<std::vector<Cell>> previous(adjacent_cells.size(), std::vector<Cell>(adjacent_cells.size(), Cell()));
    ...
    while (!processed.empty()) {
        std::sort(processed.begin(), processed.end(), [&board](std::pair<int, Cell>& left, std::pair<int, Cell>& right)
            {
        ...
        for (int i = 0; i < adjacent_cells[coordinates.first][coordinates.second].size(); ++i) {
            ...
            if (distances[neighbor_coordinates.first][neighbor_coordinates.second]
                ...
                distances[neighbor_coordinates.first][neighbor_coordinates.second] = distances[coordinates.first][coordinates.second] + neighbor_cost;
                // Maybe change
                previous[neighbor_coordinates.first][neighbor_coordinates.second] = vertex;

                // Change priority
                processed.push_back(std::make_pair(distances[neighbor_coordinates.first][neighbor_coordinates.second], neighbor));
    ...
    std::vector<Cell> path;
    std::pair<int, int> destination_coordinats = board.GetCellCoordinats(destination_cell);

    // Change for black and white case
    // If there are no path to destination cell
    if (distances[destination_coordinats.first][destination_coordinats.second] == max) {
    ...
    Cell previous_cell = board.GetCell(destination_coordinats.first, destination_coordinats.second);
    while (previous_cell != source_cell) {
        ...
        previous_cell = previous[previous_cell_coordinates.first][previous_cell_coordinates.second];
